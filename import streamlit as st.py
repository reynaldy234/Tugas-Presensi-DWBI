...
st.caption("Visualisasi Bullet Chart: Perbandingan Aktual vs Target dengan Skema Warna Gradien.")
...
def load_data():
    try:
        ...
        st.error("❌ File 'sales_2023.csv' tidak ditemukan. Pastikan file berada di direktori yang sama dengan script ini.")
        return pd.DataFrame()
...
# --- 2. SKEMA WARNA DAN RENTANG PERFORMA ---
PERFORMANCE_TIERS = [
...
# --- 3. FUNGSI PEMBUATAN BULLET CHART ---
def create_annual_bullet_chart(kpi_name, kpi_data, df_bands):
    """
    Membuat Bullet Chart Altair untuk KPI tertentu
    """
    ...
    x_axis = alt.Axis(format=format_string, labels=True)

    # Latar belakang (tier performa)
    bands_chart = alt.Chart(bands).mark_bar(height=30).encode(
        x=alt.X('start', title=None, axis=None, scale=x_scale),
        x2='end',
    ...
    target_line = alt.Chart(data).mark_tick(
        ...
        x=alt.X('Target', title=None, axis=None, scale=x_scale),
        y=y_encoding,
    ...
    actual_bar = alt.Chart(data).mark_bar(height=25, color='#1e90ff').encode(
        x=alt.X('Actual', title=None, axis=x_axis, scale=x_scale),
        y=y_encoding,
